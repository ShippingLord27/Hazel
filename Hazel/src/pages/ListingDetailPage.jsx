...
const handleOptionChange = (e) => {
    setRentalOptions(prev => ({ ...prev, [e.target.name]: e.target.value }));
};
...
const handleAddToCart = () => {
    ...
    if (currentUser.email === product.ownerId) {
        showToast("You cannot rent your own item.");
        return;
    }
    if (!rentalOptions.startDate) {
        showToast("Please select a rental start date.");
        return;
    }
...
if (!product) {
    return <div style={{ paddingTop: '100px', textAlign: 'center' }}>Loading listing...</div>;
}
